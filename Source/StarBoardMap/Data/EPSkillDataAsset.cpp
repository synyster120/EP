...
bool UEPSkillDataAsset::GetProjectileData_Implementation(int32 PhaseIndex, FEPProjectileData& OutProjectileData) const
{
...
// CreateSkill 에서 이미 로드된 생태. 가져와서 사용만 함
bool UEPSkillDataAsset::GetPhaseDataFromAsset_Implementation(int32 PhaseIndex, FEPSkillPhaseData& OutSkillPhaseData) const
{
    // 자신의 멤버 변수인 SkillData에서 ComboSequence 배열 가져옴
    if (SkillData.ComboSequence.IsValidIndex(PhaseIndex))
    {
        ...
        if (PhaseDataPtr)
        {
            OutSkillPhaseData = *PhaseDataPtr;
            return true; // 성공 반환
        }
    }
