...
void UEPSkillComponent::BeginPlay()
{
    ...
    // 이 컴포넌트의 소유자(캐릭터)를 찾아 StatComponent를 가져옵니다.
    ...
    // StatComponent는 반드시 존재해야 하므로, 없다면 에러를 발생시켜 문제를 즉시 인지하게 합니다.
...
{
    // 맵을 사용해 O(1) 시간 복잡도로 인덱스를 즉시 찾는다.
    if (const int32* IndexPtr = SkillIDToIndexMap.Find(SkillID))
    {
        int32 SkillIndex = *IndexPtr;
        FSkillRuntimeData& SkillData = SkillSlots[SkillIndex];

        // 이제 SkillData.CooldownTimerHandle을 사용해 타이머를 설정...

        // 스킬 데이터에서 쿨타임 정보를 가져옵니다.
